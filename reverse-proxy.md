...
If the reverse proxy is installed on the same host where AIO is running, you should now limit the apache container to only listen on localhost.

You can do so by running the following iptables commands:
```bash
# IPv4
sudo iptables -A DOCKER-USER -p tcp -m tcp --dport 11000 -j DROP
sudo iptables-save > /etc/iptables/rules.v4
# IPv6
sudo ip6tables -A DOCKER-USER -p tcp -m tcp --dport 11000 -j DROP
sudo ip6tables-save > /etc/iptables/rules.v6
```
For the commands above to work, you need to have iptables and ip6tables installed. Also make sure to adjust the port 11000 to match the chosen APACHE_PORT.
